...
// 2. 메인 App 컴포넌트
function ListPage({lions, setLions}) {
  ...
  const updateParams = (key, value, defaultValue) => {
    const next = new URLSearchParams(searchParams); // 기존 파라미터 복사
    if (value === defaultValue) {
        next.delete(key);   // 기본값이면 URL에서 제거 (지침 4번)
    } else {
        next.set(key, value);
    }
    ...
    <div className="container">
      ...
      <header className="control-panel">
        ...
        <div className="view-options">
          ...
          <select id="filter-part" value={filterPart} onChange={(e) => updateParams('part', e.target.value, 'all')}>
          ...
          <select id="sort-order" value={sortOrder} onChange={(e) => updateParams('sort', e.target.value, 'latest')}>
